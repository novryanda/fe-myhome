...
import { Bed, Home, Loader2, Search, User } from "lucide-react";
...
export function GlobalSearch() {
  ...
  return (
    <CommandDialog open={open} onOpenChange={setOpen}>
      <CommandInput placeholder="Cari pengguna, properti, atau kamar..." value={query} onValueChange={setQuery} />
      <CommandList>
        {isFetching && (
        ...
        {data?.users?.length > 0 && (
          <CommandGroup heading="Pengguna">
            {data.users.map((user: any) => (
              <CommandItem key={user.id} onSelect={() => onSelect(`/dashboard/user`)}>
        ...
        {data?.properties?.length > 0 && (
          <CommandGroup heading="Properti">
            {data.properties.map((property: any) => (
              <CommandItem key={property.id} onSelect={() => onSelect(`/dashboard/properties`)}>
        ...
        {data?.roomTypes?.length > 0 && (
          <CommandGroup heading="Tipe Kamar">
            {data.roomTypes.map((room: any) => (
              <CommandItem key={room.id} onSelect={() => onSelect(`/dashboard/rooms`)}>
