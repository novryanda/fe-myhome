...
import { Building2, Camera, Car, Image as ImageIcon, LayoutGrid, Palmtree } from "lucide-react";
    ...
    <div className="space-y-4">
      <Tabs value={selectedCategory} onValueChange={setSelectedCategory} className="w-full">
        <TabsList className="bg-muted/50 p-1">
          <TabsTrigger value="ALL" className="text-xs uppercase tracking-wider font-bold">
            Semua
          </TabsTrigger>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-2 h-[300px] md:h-[450px]">
                    ...
                    {count > 3 && (
                      <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                        <Button
                          variant="secondary"
                          size="sm"
                          className="h-9 px-4 font-bold rounded-lg shadow-xl backdrop-blur-md bg-white/20 text-white border-white/30 hover:bg-white/40 transition-all"
                        >
            ...
            <div className="md:hidden absolute bottom-4 right-4 z-10">
              <Button
                variant="secondary"
                size="sm"
                className="h-9 px-4 font-bold rounded-lg shadow-xl backdrop-blur-md bg-black/50 text-white border-white/10"
              >
                <LayoutGrid className="size-4 mr-2" />1 / {count}
              </Button>
            </div>
          </div>
        )}
