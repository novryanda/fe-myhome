...
export function RulesManager({ roomTypeId, initialRules }: RulesManagerProps) {
  ...
  const addMutation = useMutation({
    ...
    onError: (err: any) => toast.error(err.response?.data?.message || "Gagal menambahkan aturan"),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: any) => toast.error(err.response?.data?.message || "Gagal menghapus aturan"),
  });
  ...
  const handleSuggestionClick = (name: string) => {
    if (rules.some((r) => r.name.toLowerCase() === name.toLowerCase())) {
      ...
      const existing = rules.find((r) => r.name.toLowerCase() === name.toLowerCase());
  ...
  const isSelected = (name: string) => rules.some((r) => r.name.toLowerCase() === name.toLowerCase());
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-3">
        ...
        />
        <Button onClick={handleAdd} disabled={addMutation.isPending || !newRule.trim()}>
      ...
      {/* Suggestion chips */}
      <div>
        <p className="text-sm font-medium text-muted-foreground mb-3">Pilih aturan umum atau tambahkan sendiri:</p>
      ...
      {rules.filter((r) => !DEFAULT_SUGGESTIONS.some((s) => s.toLowerCase() === r.name.toLowerCase())).length > 0 && (
        <div>
          <p className="text-sm font-medium text-muted-foreground mb-3">Aturan kustom:</p>
          <div className="flex flex-wrap gap-2">
            {rules
              .filter((r) => !DEFAULT_SUGGESTIONS.some((s) => s.toLowerCase() === r.name.toLowerCase()))
              .map((rule) => (
                <button
                  key={rule.id}
                  type="button"
      ...
      {rules.length === 0 && (
        <div className="rounded-xl border-2 border-dashed py-12 text-center">
          <p className="text-sm text-muted-foreground">Belum ada peraturan. Pilih dari saran atau tambahkan sendiri.</p>
        </div>
      )}
    </div>
  );
