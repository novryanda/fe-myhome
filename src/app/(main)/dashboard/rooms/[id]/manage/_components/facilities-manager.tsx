...
import { CheckCircle2, Loader2, Trash2, Wifi } from "lucide-react";
...
export function FacilitiesManager({ roomTypeId, initialFacilities }: FacilitiesManagerProps) {
  ...
  const addMutation = useMutation({
    ...
    onError: (err: any) => toast.error(err.response?.data?.message || "Gagal menambahkan fasilitas"),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: any) => toast.error(err.response?.data?.message || "Gagal menghapus fasilitas"),
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm ? (
            ...
            <form onSubmit={form.handleSubmit((v) => addMutation.mutate(v))} className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Misal: Wifi, AC, Kasur" className="rounded-xl" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        Tagline <span className="text-muted-foreground font-normal">(opsional)</span>
                      </FormLabel>
                      <FormControl>
                        <Input placeholder="Keterangan singkat" className="rounded-xl" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      URL Ikon <span className="text-muted-foreground font-normal">(opsional)</span>
                    </FormLabel>
                    <FormControl>
                      <Input placeholder="https://example.com/icon.svg" className="rounded-xl" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
        ...
        <Button onClick={() => setShowForm(true)} className="rounded-xl bg-emerald-950 hover:bg-emerald-900">
          Tambah Fasilitas
        </Button>
      )}
          ...
          {facilities.map((facility) => (
            ...
            >
              <div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-emerald-700">
                {facility.iconUrl ? (
                  <img src={facility.iconUrl} alt={facility.name} className="size-5 object-contain" />
                ) : (
              ...
              <div className="min-w-0 flex-1">
                ...
                {facility.tagline && <p className="text-xs text-muted-foreground truncate">{facility.tagline}</p>}
              </div>
