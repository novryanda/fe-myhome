...
import { AlertCircle, ArrowLeft, ChevronRight, Trash2 } from "lucide-react";
...
export function ManageRoomTypeClient({ roomTypeId }: ManageRoomTypeClientProps) {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: any) => toast.error(err.response?.data?.message || "Gagal menghapus tipe kamar"),
  });
  ...
  if (!room) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center border rounded-2xl border-dashed">
        ...
        <p className="text-muted-foreground mb-4 max-w-sm">Data tidak ditemukan atau Anda tidak memiliki akses.</p>
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Semua data termasuk kamar, fasilitas, dan peraturan akan dihapus permanen. Tindakan ini tidak dapat
                dibatalkan.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        <Tabs defaultValue="info" className="w-full">
          ...
          <div className="p-6">
            ...
            <TabsContent value="facilities" className="mt-0">
              <FacilitiesManager roomTypeId={room.id} initialFacilities={room.facilities || []} />
            </TabsContent>
          </div>
        </Tabs>
      </div>
