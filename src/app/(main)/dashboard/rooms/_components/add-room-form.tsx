    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit((v) => mutation.mutate(v), handleInvalidSubmit)} className="space-y-6">
        ...
        <nav className="flex items-center gap-1.5 text-sm text-muted-foreground">
          ...
          <span className="font-medium text-foreground">{isEditMode ? "Edit Tipe Kamar" : "Tambah Tipe Kamar"}</span>
        </nav>

        {/* Header */}
        <h1 className="text-2xl font-bold tracking-tight">{isEditMode ? "Edit Tipe Kamar" : "Tambah Tipe Kamar"}</h1>
            ...
            <div className="p-6">
              ...
              <TabsContent value="info" className="mt-0 space-y-8">
                        ...
                        <SelectContent>
                          ...
                          {properties.length === 0 && !isLoadingProperties && (
                            <div className="p-2 text-sm text-muted-foreground text-center">
                              Tidak ada properti yang di-approve.
                            </div>
                          )}
                        </SelectContent>
                      </Select>
                ...
                <div className="space-y-4">
                  ...
                  <PriceRow
                    form={form}
                    toggleName="is3Monthly"
                    priceName="price3Monthly"
                    label="Sewa 3 Bulan"
                  ...
                  <PriceRow
                    form={form}
                    toggleName="isDepositRequired"
                    priceName="depositAmount"
                    label="Wajib Deposit?"
              ...
              <TabsContent value="rules" className="mt-0">
                <RulesTab rules={form.watch("rules") || []} onAdd={addRule} onRemove={removeRule} />
              </TabsContent>
...
function PriceRow({
  ...
  return (
    <div
      className={`flex flex-col md:flex-row md:items-center gap-4 rounded-xl border p-4 ${highlighted ? "bg-muted/20" : ""} ${destructive ? "border-destructive/20" : ""}`}
    >
      <FormField
        control={form.control}
        name={toggleName}
      ...
      {showForm ? (
        <div className="rounded-xl border bg-muted/30 p-5 space-y-4">
          ...
          <div className="flex justify-center gap-4">
            ...
            <Button type="button" onClick={handleSubmit} disabled={!ruleName.trim()}>
              Submit
            </Button>
          </div>
        </div>
    ...
    <div className="space-y-6">
      {showForm ? (
        <div className="rounded-xl border bg-muted/30 p-5 space-y-4">
          ...
          <div>
            <label className="text-sm font-medium mb-1.5 block">
              Ikon <span className="text-muted-foreground font-normal">(opsional)</span>
            </label>
          ...
          <div className="space-y-3">
            {facilities.map((f, index) => (
              ...
              >
                <div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-emerald-700">
                  {f.iconUrl ? (
                    <img src={f.iconUrl} alt={f.name} className="size-5 object-contain" />
                  ) : (
